...
let rec map_suite_m s ~f =
  ...
  | Emp -> Ok Emp
  | Ext (s', x) ->
     let* sm' = map_suite_m s' ~f in
     let* xm = f x in
     Ok (Ext (sm', xm))

let rec split_last s =
  match s with
  | Emp -> Error "List too short"
  | Ext (Emp, x) -> Ok (x, Emp)
  | Ext (xs, x) ->
     let* (y, ys) = split_last xs in
     Ok (y, Ext (ys, x))
...
let rec split (k : int) (s : 'a suite) =
  let open Base.Result.Monad_infix in
  if k <= 0 then Ok (s, Emp) else
  match s with
  | Emp -> Error "Index out of range"
  | Ext (ys, y) ->
     split (k-1) ys >>= fun (xs,xs') ->
     Ok ((xs , Ext (xs', y)))
...
let rec range a b =
  if a >= b then Emp else Ext (range (a+1) b, a)
